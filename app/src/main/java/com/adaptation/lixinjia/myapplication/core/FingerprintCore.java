package com.adaptation.lixinjia.myapplication.core;
...
public class FingerprintCore {
    ...
    public FingerprintCore(Context context) {
        ...
        Log.d("FingerprintCore","fingerprint isSupport: " + isSupport);
        initCryptoObject();
    }
    ...
    private void initCryptoObject() {
        try {
            ...
            Log.d("FingerprintCore","创建cryptoobject失败!");
        }
    }
    ...
    private void notifyAuthenticationError(int errMsgId, CharSequence errString) {
        Log.d("FingerprintCore","onAuthenticationError, 错误Id:" + errMsgId + ", 错误:" + errString + ", 30秒后重试");
        if (null != mFpResultListener && null != mFpResultListener.get()) {
    ...
    private void notifyAuthenticationFailed(int msgId, String errString) {
        Log.d("FingerprintCore","onAuthenticationFailed, msdId: " +  msgId + " 错误: " + errString);
        if (null != mFpResultListener && null != mFpResultListener.get()) {
    ...
    public void cancelAuthenticate() {
        if (mCancellationSignal != null && mState != CANCEL) {
            Log.d("FingerprintCore","cancelAuthenticate...");
            mState = CANCEL;
    ...
    private void onFailedRetry(int msgId) {
        mFailedTimes++;
        Log.d("FingerprintCore","失败的重试时间 " + mFailedTimes);
        cancelAuthenticate();
    ...
    public static FingerprintManager getFingerprintManager(Context context) {
        ...
        try {
            ...
            Log.d("FingerprintCore","have not class FingerprintManager");
        }
